...
SYNC_FILES="CODE_OF_CONDUCT.md LICENSE Makefile.common SECURITY.md .yamllint golangci-lint.yml"
...
process_repo() {
  ...
  for source_file in ${SYNC_FILES}; do
    ...
    if [[ "${source_file}" == 'golangci-lint.yml' ]] && ! check_go "${org_repo}" "${default_branch}" ; then
    ...
    if [[ "${source_file}" == 'golangci-lint.yml' ]] ; then
